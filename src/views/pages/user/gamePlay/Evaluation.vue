    ...
    <div class="d-flex flex-column gap-4 w-100" v-if="ready">
      <div class="border-bottom border-secondary">
        <h1 class="h4 fw-bolder text-uppercase">
          evaluasi
          {{
            Object(appStore.appCacheData.pembelajaran.subContent)[
      ...
      <div
        v-if="
          Object(userStore.user.appData.evaluation)[`${route.query.evaluation}`]
        "
      >
        <div class="container">
      ...
      <div
        v-if="
          !Object(userStore.user.appData.evaluation)[
      ...
      >
        <div
          ...
          v-for="(question, i) in appStore.appCacheData[
            `${useRoute().params.playingContent}`
          ].evaluation[`${useRoute().query.evaluation}`]"
          :key="i"
        >
          <div class="container">
            <div class="d-flex">
...
const sendEvaluation = () => {
  const question = Object.keys(
    appStore.appCacheData[`${route.params.playingContent}`].evaluation[
      `${route.query.evaluation}`
    ]
  );
  if (
    question.length >
    Object.keys(evaluation[`${route.query.evaluation}`]).length
  ) {
